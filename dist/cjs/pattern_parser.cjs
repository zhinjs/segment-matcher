...
const pattern_token_1 = require("./pattern_token.cjs");
/**
 * 模式解析器类
 */
class PatternParser {
    ...
    static parseTypedLiteral(pattern, startIndex) {
        ...
        if (colonIdx !== -1) {
            ...
            return pattern_token_1.PatternToken.createTypedLiteral(type, value);
        }
        // 如果没有冒号，整个内容作为类型
        return pattern_token_1.PatternToken.createTypedLiteral(content.trim(), '');
    }
    static parseRequiredParameter(pattern, startIndex) {
        ...
        return pattern_token_1.PatternToken.createParameter(name.trim(), type ? type.trim() : 'text', false);
    }
    static parseOptionalParameter(pattern, startIndex) {
        ...
        if (trimmedName.startsWith('...')) {
            ...
            return pattern_token_1.PatternToken.createRestParameter(restName, typeAndDefault || null);
        }
        ...
        return pattern_token_1.PatternToken.createParameter(trimmedName, dataType || 'text', true, defaultValue);
    }
    ...
    static parseLiteral(pattern, startIndex) {
        ...
        // 只判断是否为空，不要trim内容
        return {
            token: literal ? pattern_token_1.PatternToken.createLiteral(literal) : null,
            newIndex: i
        };
    }
